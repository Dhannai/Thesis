...
%The study site, Pescadero Estuary, is located in San Mateo County, California and has experienced some strong wind events during its closed state.
...
% \textit{Breve, 1 o 2 páginas, para dar una idea general del contexto del trabajo, y por qué es importante de ser analizado. Es muy importante que la primera frase apunte a una idea global de por qué el problema es relevante, para en los párrafos siguientes ir acercándose cada vez mas a lo específico.}\\
...
% There is a considerable amount of research concerning the processes that maintain an open inlet for larger, more tidally dominated systems (\cite{Behrens2013}). In the literature there are several studies that have modeled the processes of rise and overflow of the waves, but there is a lack of studies related to the breaking of the barrier. These models could be modified to predict a break, but may only work for beaches with similar morphology (\cite{Orescanin2018}). Relatively little work has investigated the controls on estuary breaching and closing, especially of smaller, bar-built estuarine systems. For this reason, it is important to study the opening of the sand barrier, under what conditions of flow, stratification and wind occurs, and the effects in these types of estuaries, since they present a drastic change in the shape of the beach.

% En su estado de desconexión al océano el estuario puede tomar forma de laguna estratificada, la cual tiene como principal fuente de variabilidad a corto plazo la surgencia y/o los seiches formados por el esfuerzo superficial del viento. 


% Previous studies have designed models of processes that describe the closure of the sand barrier (\cite{Bertin2019}; \cite{McSweeney2018}; \cite{Safak2016}). Besides, there is literature on the rise and overflow of the waves on the sand barrier (\cite{Behrens2013}), which would be closer to, but not exactly the opening of the inlet. There are also more recent works (\cite{Orescanin2018}) that have modeled, through momentum balance, the discharge at the breach, but without considering external factors.
...
% \subsubsection{Investigation opportunity}

% In this context, based on the previous studies, it is identified as an opportunity for improvement in the field, to analyse the previous physical processes that cause the opening of estuaries inlet. In order to achieve this we can verify the effect of the winds on the sandbar eroding, also we can study the upwelling caused by this wind stress and its dynamics effects using depth, salinity and temperature data. Besides, we can built a mathematical model to simulate what happens before and during the opening of the mouth, to be tested with data from different estuaries with similar dynamics and observe under what characteristics the opening of the mouth occurs due to the wind.

% In addition, we can analyse the effects of resuspension and transport of sediments after inlet breach in these environments with high stratification, analyzing the biogeochemical processes and the sedimentation of polluting particles that may affect the environmental health, the marine life of the place and the well-being of the people.

% \subsubsection{Hypothesis}
% In Mediterranean climates, where seasonal changes are present, during the rainy months there is a flow increase in rivers, which reach the sea through coastal water bodies like estuaries, fjords or coastal lagoons. Small tidal inlets in bar-built estuaries, are exposed to flow changes, which make these systems particularly dynamic due to the sandbank morphology that they have. In dry months, the closure of intermittent inlets is observed in these regions, and during rainy seasons the mouth opens.

% Inlet opening can be driven by many factors that remain partly understood, one of them is wind stress. When there is a certain wind speed the surface tilts (Fig. \ref{fig: wind}), accumulating potential energy where the water level rises $\delta$ (\cite{Shintani2010}; \cite{Behrens2016}; \cite{Imberger1982}). Once the wind stops the system tries to reach equilibrium, meaning that if the sandbar is on the opposite side, the accumulated energy can erode the bar and open the inlet. 

% \begin{figure}[H]
%     \centering
%     \includegraphics[scale=0.6]{Imagenes/wind_stress.png}
%     \caption{Wind stress effects in the estuary}
%     \label{fig: wind}
% \end{figure}

% On the other hand, stratification present in bar-built estuaries can also be affected by wind. Surface stress moves the upper layer (Fig. \ref{fig: wind}) displacing the lower layer. This can increase the pressure on the sandbar and could influence the inlet opening.

% The present study aims at verifying these hypothesis, based on a field data set collected on Pescadero Estuary presented at \cite{Williams2014}.

\section{Objectives}

% \textit{Un objetivo general, y tres o cuatro objetivos específicos. Deben ser declaraciones simples, que puedan ser verificadas al final del trabajo. La idea es que el análisis del estado del arte permita justificar los objetivos.}

\subsection{General objective}

% The mean goal of the present work is to identify and understand the physical factors leading to opening in a small tidal inlet, focusing on wind-effects at different levels of stratification, in order to verify wind stress role at this process. Our case study is the Pescadero Estuary, a bar-built estuary in California that represents many other small inlet systems elsewhere in the word. Two data sets of wind and pressure at this site containing several mouth openings and closures are going to be used.
...
% \textit{Un análisis más detallado de lo que se conoce respecto del tema, mas o menos en este orden: Por qué es importante, como se ha atacado o enfocado su análisis en otras partes del mundo y también en Chile. Formas de medir, evaluar, cuantificar, etc, si es que aplica. Es importante en esta sección identificar claramente lo que falta por hacer, y declarar cómo este trabajo aporta a resolver alguno de los aspectos deficitarios. También, si aplica, se explica que evidencia hay de que lo que se está analizando sea importante en algún lugar en específico, y por que es interesante de ser estudiado y analizado. En esta sección se citan papers y todo el conocimiento previo que sea relevante. Al final del capítulo, debe quedar claro la justificación de la pertinencia de tu trabajo, lo que da una base clara para poder declarar tus objetivos.}

% \subsection{Arguments that expose the importance of bar-built estuaries in ecosystem and economics }
...
% \subsection{Arguments of how bar-built estuaries are studied around the world and in chile}
...
% \subsection{Que falta por hacer y que aporta este trabajo}
...
% wind:
% Studies of wind effects have often focused on big lakes with thermal stratification (\cite{roberts2021setup}), big estuaries (\cite{janzen2002wind}) or in the case of bar built estuaries in its open state \cite{payandeh2019subtidal},

% it was observed that the results of the influence of wind depend in the geomorphology of estuaries \cite{payandeh2019subtidal}.

% Some studies suggest that local wind-effects are more important for shallow estuaries that the deeper ones \cite{huang2011hydrodynamic}

% \cite{janzen2002wind} suggest that local forcing winds are more important for estuary-shelf exchange in big estuaries studies and shows that orientation of main directions of wind have an important role on estuarine currents

\section{Methods}

% \textit{Aquí se describe el trabajo que se hará, con especial énfasis en identificar los pasos y métodos que te ayudarán a cumplir tus objetivos. Idealmente, se debe relacionar la metodología con algo que ya describiste en el estado del arte, como para indicar que no estás inventando un método (de no ser que la memoria sea justamente la propuesta de un método). Acá se describen también las fuentes de datos, etc. así como los métodos de procesamiento de ellos, comparación de error, etc.}
...
% \textit{Acá presentas claramente los resultados de la aplicación de la metodología, a través de texto y figuras. Un aspecto importante es que texto y figuras deben ir de la mano en todo el trabajo. Es decir, cada vez que se incluye una figura, es porque aporta a la comprensión de lo que se dice en el texto, y por lo tanto, se debe mencionar en éste ultimo. No coloquen figuras sueltas esperando que se autoexpliquen. Tampoco usen el texto de la figura para introducir ideas. Las figuras son fundamentales para la comprensión del texto, pero deben ir de la mano con él.}

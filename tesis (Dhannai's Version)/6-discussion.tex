...
The density time-series were showing a decrease in density in time, specially at the bottom layer in NM and ML (Fig. \ref{fig:dens}), in DC we also observed a decrease but not in the deepest layer, in which we observed a light increase of density in the first period. The density decrease in time indicates a constant freshwater input. In Fig. \ref{fig:diff} for $\Delta \rho / \delta z$ we does not observe a constant change in time as in $\rho$ 





en d rho/dz se ven cambios de densidad solo en eventos de viento, pero se observa que cada vez afectan menos los eventos de viento
